...
export const RulesEditor: React.FC = () => {
  ...
  const addRule = () => {
    ...
    setRules(config.Config.createFrom({
      ...rules,
  ...
  const updateRule = (index: number, field: string, value: string | string[]) => {
    if (!rules) return;
      ...
      <div className="flex items-center justify-center px-4 py-8">
        <Card className="w-full max-w-4xl">
          <CardHeader>
            ...
            <CardDescription>Load a configuration file to edit rules</CardDescription>
          </CardHeader>
        </Card>
      </div>
    );
      ...
      <Card className="w-full max-w-6xl flex flex-col h-[600px]">
        <CardHeader className="flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Rules Editor</CardTitle>
              <CardDescription>Edit and manage your download rules</CardDescription>
            </div>
              ...
              rules.Rules.map((rule, index) => {
                ...
                return (
                  <Card key={index} className="p-4">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      >
                        ...
                        <h3 className="text-lg font-semibold">
                          Rule {index + 1}: {rule.Name || `Unnamed Rule ${index + 1}`}
                        </h3>
                      </button>
                      <Button
                        onClick={(e) => {
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`rule-folder-${index}`}>Folder (optional)</Label>
                      <Input
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`rule-title-${index}`}>Title (optional)</Label>
                      <Input
                        id={`rule-title-${index}`}
                        value={rule.Title || ''}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`rule-keyword-${index}`}>Keyword (optional)</Label>
                      <Input
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`rule-pfm-${index}`}>Personality/Performer (optional)</Label>
                      <Input
                        id={`rule-pfm-${index}`}
                        value={rule.Pfm || ''}
                        onChange={(e) => updateRule(index, 'Pfm', e.target.value)}
                        placeholder="DJ/MC name"
                      />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor={`rule-station-${index}`}>Station ID (optional)</Label>
                      <Input
                        id={`rule-station-${index}`}
                        value={rule.StationID || ''}
                        onChange={(e) => updateRule(index, 'StationID', e.target.value)}
                        placeholder="e.g., FMT"
                      />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor={`rule-window-${index}`}>Time Window (optional)</Label>
                      <Input
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`rule-dow-${index}`}>Days of Week (optional)</Label>
                      <Input
                        id={`rule-dow-${index}`}
                        value={Array.isArray(rule.DoW) ? rule.DoW.join(', ') : ''}
                        onChange={(e) => {
